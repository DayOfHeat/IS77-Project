...
## Data formatting & cleaning (Due March 29)

Both the fraud data and the labour data are completely imported and ready for use. Fraud data had missing values handled, which the labor data didn't need, however, the sheets each had inconsistent formatting and time periods which had to be standardized before the data could be used. They can be found in `project.ipynb`.

## Data integration (Due April 5)
...
## Data Analysis (Due April 12)

We are currently figuring out the best way to analyze the data, so right now our only progress in this section is manual analysis based on look at graphs.

## Data Visualization (Due April 19)

We produced a graphs showing trends in benefits fraud over time and total fraud vs unemployment as some preliminary visualization to help figure out how we want to present the data. They can be found in `project.ipynb`. Neither graph is final yet and we plan to add more in the final report.

## Finish report (Due May 7)
